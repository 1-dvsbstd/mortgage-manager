(() => {
  ...
  function render() {
    ...
    const base = result.base; const accelerated = result.accelerated;
    ...
    const cssHeight = expanded ? (compact ? 390 : Math.max(360, Math.min(500, Math.round(cssWidth * 0.46)))) : (cssWidth < 620 ? 250 : 315);
    ...
    for (let i = 0; i <= 4; i += 1) {
      ...
      ctx.strokeStyle = 'rgba(255,255,255,.075)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(pad.left, y); ctx.lineTo(pad.left + width, y); ctx.stroke();
      ctx.fillStyle = '#8fa39c'; ctx.textAlign = 'right'; ctx.fillText(compactMoney(maxValue * (1 - i / 4)), pad.left - 10, y);
    }
    ...
    yearTicks.forEach(({ month, label }, index) => {
      const x = xFor(month);
      if (index > 0 && index < yearTicks.length - 1) { ctx.strokeStyle = 'rgba(255,255,255,.035)'; ctx.beginPath(); ctx.moveTo(x, pad.top); ctx.lineTo(x, pad.top + height); ctx.stroke(); }
      ctx.fillStyle = '#8fa39c'; ctx.textAlign = index === 0 ? 'left' : index === yearTicks.length - 1 ? 'right' : 'center'; ctx.fillText(label, x, cssHeight - 17);
    });
    ...
    if (fixedMonth !== null && fixedMonth >= 0 && fixedMonth <= maxMonths) {
      ...
      ctx.save(); ctx.setLineDash([4, 5]); ctx.strokeStyle = 'rgba(231,220,196,.42)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(x, pad.top); ctx.lineTo(x, pad.top + height); ctx.stroke(); ctx.restore();
      ctx.fillStyle = '#d7cdb8'; ctx.font = compact ? '9px system-ui' : '10px system-ui'; ctx.textAlign = x > cssWidth * .72 ? 'right' : 'left'; ctx.fillText('Fix ends', x + (x > cssWidth * .72 ? -5 : 5), pad.top + 13);
    }

    const readout = ensureReadout();
    if (hoverMonth === null) { readout.innerHTML = `<span>${compact ? 'Tap' : 'Hover or tap'} the chart to inspect mortgage and equity over time.</span>`; return; }
    const month = Math.max(0, Math.min(maxMonths, hoverMonth)); const x = xFor(month);
    const baseBalance = pointAt(base.monthlyPoints, month); const overBalance = pointAt(accelerated.monthlyPoints, month); const equity = pointAt(equityPoints, month);
    ctx.strokeStyle = 'rgba(255,255,255,.28)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(x, pad.top); ctx.lineTo(x, pad.top + height); ctx.stroke();
    const dot = (value, colour) => { ctx.fillStyle = colour; ctx.beginPath(); ctx.arc(x, yFor(value), 4, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = '#071713'; ctx.lineWidth = 2; ctx.stroke(); };
    dot(baseBalance, '#a8b4b0'); dot(overBalance, '#54e0b4'); if (equityPoints.length) dot(equity, '#e7dcc4');
    const projectedHome = v.homeValue > 0 ? v.homeValue * Math.pow(1 + homeTrend()/100, month/12) : 0;
    const ltv = projectedHome > 0 ? (baseBalance / projectedHome) * 100 : null; const difference = Math.max(0, baseBalance - overBalance);
  ...
  function monthFromPointer(event) {
    ...
    const v = values(); const result = MortgageMath.compare(v.balance, v.rate, v.payment, v.extra);
  ...
  document.addEventListener('click', (event) => { if (event.target.closest('[data-expandable-card="trajectory"]')) scheduleRender(); });
  ...
  if (card) new MutationObserver(scheduleRender).observe(card, { attributes: true, attributeFilter: ['class', 'aria-expanded'] });
  scheduleRender();
})();
