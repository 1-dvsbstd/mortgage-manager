(() => {
  ...
  function ensureUI(){
    ...
    section.id='homeProjection'; section.className='home-projection simplified';
    section.innerHTML=`
      ...
      <details class="projection-assumptions"><summary>Estimate settings</summary><div class="projection-controls purchase-controls">
      ...
      </div><p class="deep-note">If you add a recent valuation or agent estimate, that becomes the starting value today. Otherwise the model grows your purchase price from the month you bought and adds any improvement value you enter. Local comparable sales are not included yet.</p></details>`;
  ...
  function render(){
    ...
    const balance=Math.max(0,Number(mortgage.balance)||0), rate=Math.max(0,Number(mortgage.rate)||0), payment=Math.max(0,Number(mortgage.payment)||0), extra=Math.max(0,Number(mortgage.scenarioExtra)||0);
    ...
    let modelledToday=0; if(validPurchase) modelledToday=projectedValue(purchasePrice,settings.trend,yearsSince(purchaseMonth))+improvements;
