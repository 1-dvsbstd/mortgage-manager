(() => {
  ...
  const compactMonths = (months) => { if(!Number.isFinite(months)||months<=0)return 'now'; const y=Math.floor(months/12),m=months%12; return y&&m?`${y}y ${m}m`:y?`${y}y`:`${m}m`; };

  function monthsUntil(monthValue){
    if(!monthValue)return null;
    const [year,month]=String(monthValue).split('-').map(Number); if(!year||!month)return null;
  ...
  function balanceAt(points,month){ if(!Array.isArray(points)||!points.length)return 0; return points[Math.min(Math.max(0,month),points.length-1)]??0; }
  function projectedBalanceWithExtra(state,months,additionalMonthly){
    if(!window.MortgageMath)return Number(state.balance)||0;
    const payment=Math.max(0,Number(state.payment)||0)+Math.max(0,Number(state.currentOverpayment)||0)+Math.max(0,Number(additionalMonthly)||0);
    return balanceAt(MortgageMath.amortize(state.balance,state.rate,payment).monthlyPoints,months);
  }
  function monthlyExtraForTarget(state,months,targetBalance){
    if(months<=0)return null;
    const currentEndBalance=projectedBalanceWithExtra(state,months,0); if(currentEndBalance<=targetBalance)return 0;
    let low=0,high=100; while(high<10000&&projectedBalanceWithExtra(state,months,high)>targetBalance)high*=2;
    if(high>=10000&&projectedBalanceWithExtra(state,months,high)>targetBalance)return null;
    for(let i=0;i<28;i+=1){ const mid=(low+high)/2; if(projectedBalanceWithExtra(state,months,mid)<=targetBalance)high=mid; else low=mid; }
    return Math.ceil(high);
  }

  function ensurePlanner(){
    const panel=document.querySelector('.next-panel'), detail=document.querySelector('.next-panel .expand-detail');
    if(!panel||!detail||$('dealEndPlanner'))return;
    const section=document.createElement('section'); section.id='dealEndPlanner'; section.className='deal-end-planner';
    section.innerHTML=`
      <div class="deal-planner-heading"><div><p class="eyebrow">Deal-end plan</p><h2>Where you may stand when this fix ends</h2></div><span id="dealPlannerDate" class="source-date">—</span></div>
      <div id="dealPlannerMissing" class="deal-planner-empty" hidden></div>
      <div id="dealPlannerContent">
        <div id="dealPlannerSummary" class="deal-planner-summary">
    ...
    const heading=section.querySelector('.deal-planner-heading'), missing=section.querySelector('.deal-planner-empty'), summary=section.querySelector('.deal-planner-summary');
    panel.insertBefore(heading,detail); panel.insertBefore(missing,detail); panel.insertBefore(summary,detail);
    heading.classList.add('deal-planner-glance-heading'); summary.classList.add('deal-planner-glance');
  }
  ...
  function render(){
    ...
    if(fixedMonths===null){
      ...
      empty.innerHTML='<strong>Add your fixed-rate end date</strong><span>Once it is set, Mortgage Manager can project your balance, LTV and possible future payments at that point.</span>'; return;
    }
    if(fixedMonths<0){
      empty.hidden=false; content.hidden=true; if(summary)summary.hidden=true;
