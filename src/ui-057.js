(() => {
  ...
  function syncTileValues(){
    ...
    if(over) over.innerHTML=`<span class="current-overpay-amount">${money($('currentOverpayment')?.value||0)}</span><span class="current-overpay-suffix">/month</span>`;
  ...
  function ensurePayoffDateVisible(){
    ...
    const balance=Number($('balance')?.value)||0, rate=Number($('rate')?.value)||0, payment=Number($('payment')?.value)||0, over=Number($('currentOverpayment')?.value)||0;
  ...
  function renderEquityRows(){
    ...
    const balance=Math.max(0,Number($('balance')?.value)||0),rate=Math.max(0,Number($('rate')?.value)||0),payment=Math.max(0,Number($('payment')?.value)||0),regularOverpay=Math.max(0,Number($('currentOverpayment')?.value)||0),home=Math.max(0,Number($('homeValue')?.value)||0),ownership=Math.min(100,Math.max(0,Number($('ownership')?.value)||0));
  ...
  document.addEventListener('input',e=>{if(e.target.matches('#balance,#rate,#payment,#currentOverpayment,#homeValue,#ownership,#fixedEnd,#projectionTrendRate'))requestAnimationFrame(()=>{syncTileValues();ensurePayoffDateVisible();renderEquityRows();});});
