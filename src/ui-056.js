(() => {
  ...
  function renderCostComparison() {
    ...
    const balance=Math.max(0,Number(mortgage.balance)||0), rate=Math.max(0,Number(mortgage.rate)||0), payment=Math.max(0,Number(mortgage.payment)||0), currentOverpay=Math.max(0,Number(mortgage.currentOverpayment)||0), home=Math.max(0,Number(mortgage.homeValue)||0), ownership=Math.min(100,Math.max(0,Number(mortgage.ownership)||0));
    const path = MortgageMath.amortize(balance, rate, payment + currentOverpay);
    const settings = getHomeSettings(); const history = getHistorySummary(); const trend = getTrendRate();
    const purchasePrice = Math.max(0, Number(history.purchasePrice) || Number(settings.purchasePrice) || 0);
    const improvements = Math.max(0, Number(settings.improvements)||0); const historicalInterest = Math.max(0, Number(history.historicalInterest) || 0);
    if (!Number.isFinite(path.months)) return;
    const years = path.months / 12; const futureWholeValue = home * Math.pow(1 + trend/100, years); const futureShareValue = futureWholeValue * ownership/100;
    const futureInterest = Math.max(0, Number(path.interest)||0); const remainingPayments = balance + futureInterest; const knownBasis = purchasePrice ? purchasePrice + improvements + historicalInterest + futureInterest : 0;
    $('costFutureValue').textContent = money(futureShareValue || futureWholeValue);
    $('costFutureValueNote').textContent = ownership < 100 ? `${ownership.toFixed(ownership%1?1:0)}% share of the projected property value.` : `Projected property value using ${trend.toFixed(1)}% annual growth.`;
    ...
    if (knownBasis) {
      ...
      if (historicalInterest > 0 && history.complete) $('costComparisonNote').textContent = 'Uses your entered mortgage history for estimated past interest and the current mortgage path for future interest. Fees, maintenance, insurance and other ownership costs are still excluded.';
      else if (historicalInterest > 0) $('costComparisonNote').textContent = 'Historical interest is partially reconstructed from the deal periods entered. Add any missing mortgage periods to improve the lifetime estimate.';
      else $('costComparisonNote').textContent = 'Purchase details are included, but past mortgage interest is not yet reconstructed. Add your previous mortgage deals under Setup & data → Mortgage history.';
    } else {
      $('costKnownBasis').textContent = 'Add purchase price'; $('costKnownBasisNote').textContent = 'Purchase price and mortgage history are set under Setup & data.'; $('costComparisonNote').textContent = 'Add mortgage history to turn this into a more complete lifetime-cost estimate.';
    }
  }
  ...
  document.addEventListener('input', (event) => { if (event.target.matches('#projectionTrendRate,#projectionPurchasePrice,#projectionImprovements')) schedule(); });
  document.addEventListener('change', (event) => { if (event.target.matches('#projectionPurchasePrice,#projectionImprovements')) schedule(); });
  document.addEventListener('mortgage-history-updated', schedule);
  schedule(); setTimeout(renderCostComparison, 250);
})();
