  ...
  function openSetup() {
    ...
    backdrop.innerHTML = `
      <section class="personal-modal" role="dialog" aria-modal="true" aria-labelledby="personalModalTitle">
        <div class="personal-modal-head"><div><p class="eyebrow">Your mortgage</p><h2 id="personalModalTitle">Setup & data</h2><p>These are the numbers the whole dashboard uses. Changes save to this device.</p></div><button class="personal-close" type="button" aria-label="Close">×</button></div>
    ...
    backdrop.querySelector('[data-action="reset-history"]')?.addEventListener('click', () => {
      if (confirm('Reset all saved monthly history? Your current mortgage details will stay saved.')) {
        localStorage.removeItem(SNAPSHOT_KEY); renderProgress(); toast('History reset');
      }
    });
  ...
  function importBackup(file) {
    ...
    reader.onload = () => {
      try {
        ...
        Object.entries(payload.data).forEach(([key,value]) => { if (key.startsWith('mortgage-manager')) localStorage.setItem(key, String(value)); });
        toast('Backup imported · reloading');
        setTimeout(() => location.reload(), 600);
      } catch (_) { toast('Could not import that backup'); }
    };
